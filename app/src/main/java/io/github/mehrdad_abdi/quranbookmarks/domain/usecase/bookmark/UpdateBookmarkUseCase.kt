...
class UpdateBookmarkUseCase @Inject constructor(
    ...
    suspend operator fun invoke(bookmark: Bookmark): Result<Unit> {
        return try {
            ...
            repository.updateBookmark(bookmark.copy(updatedAt = System.currentTimeMillis()))

            // Re-schedule caching for updated bookmark (will replace existing cache)
            scheduleBookmarkCacheUseCase(bookmark.id)
